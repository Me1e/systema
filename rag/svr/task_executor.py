...
async def run_raptor(row, chat_mdl, embd_mdl, vector_size, callback=None):
    ...
    try:
        ...
        logging.exception(f"Failed to retrieve chunks for RAPTOR processing: {e}")
        raise Exception(f"RAPTOR chunk retrieval failed: {str(e)}")

    if not chunks:
        logging.warning(f"No chunks found for RAPTOR processing on document {row['doc_id']}")
        return [], 0
    ...
    try:
        ...
        logging.exception(f"RAPTOR processing failed: {e}")
        # If RAPTOR fails completely, return original chunks as fallback
        if "CONTENT_FILTERED" in str(e) or "safety" in str(e).lower():
            logging.warning("RAPTOR failed due to content filtering. Returning original chunks as fallback.")
            # Return empty result to indicate RAPTOR processing couldn't complete
            return [], 0
        else:
            # For other errors, re-raise with more context
            raise Exception(f"RAPTOR processing failed: {str(e)}")
    ...
    # Only process new chunks generated by RAPTOR
    new_chunks = chunks[original_length:] if len(chunks) > original_length else []
    
    for content, vctr in new_chunks:
        try:
            d = copy.deepcopy(doc)
            ...
            logging.exception(f"Failed to process RAPTOR chunk: {e}")
            # Continue processing other chunks even if one fails
            continue
    
    logging.info(f"RAPTOR processing completed: {len(new_chunks)} new chunks generated, {len(res)} processed successfully")
    return res, tk_count
...
async def do_handle_task(task):
    ...
    if task.get("task_type", "") == "raptor":
        ...
        try:
            ...
            error_message = f'Fail to bind chat model for RAPTOR: {str(e)}'
            progress_callback(-1, msg=error_message)
            logging.exception(error_message)
            raise
        ...
        try:
            ...
            # If RAPTOR returned empty chunks due to content filtering, handle gracefully
            if not chunks:
                progress_callback(prog=1.0, msg="RAPTOR processing completed but no new summaries were generated due to content restrictions.")
                return
                
        except Exception as e:
            error_message = f"RAPTOR processing failed: {str(e)}"
            
            # Check if it's a content filtering issue
            if "CONTENT_FILTERED" in str(e) or "safety" in str(e).lower():
                progress_callback(prog=1.0, msg="RAPTOR processing completed but content was filtered for safety reasons.")
                logging.warning(f"RAPTOR processing skipped due to content filtering for task {task_id}: {e}")
                return
            else:
                progress_callback(-1, msg=error_message)
                logging.exception(error_message)
...
async def handle_task():
    ...
    try:
        ...
        try:
            ...
            # Enhanced exception group handling
            if isinstance(e, exceptiongroup.ExceptionGroup):
                # Extract meaningful error messages from exception groups
                error_details = []
                for exc in e.exceptions:
                    if "CONTENT_FILTERED" in str(exc):
                        error_details.append("Content was blocked by safety filters")
                    elif "INVALID_REQUEST" in str(exc):
                        error_details.append("Invalid request to language model")
                    elif "finish_reason" in str(exc) and "2" in str(exc):
                        error_details.append("Response blocked by safety filters")
                    else:
                        error_details.append(str(exc))
                
                if error_details:
                    if all("safety" in detail.lower() or "filter" in detail.lower() or "block" in detail.lower() for detail in error_details):
                        err_msg = "RAPTOR processing failed due to content safety restrictions. This may be due to sensitive content in the document."
                    else:
                        err_msg = "Multiple errors occurred: " + "; ".join(error_details[:3])  # Limit to first 3 errors
                else:
                    err_msg = f"Exception group with {len(e.exceptions)} errors"
